...
\begin{figure}
    ...
    \begin{minipage}{\textwidth}
        ...
        \begin{tikzpicture}[node distance=3.0cm, font=\small, scale=0.75, every node/.style={transform shape}]
            ...
            \draw (immagine.south) -- node[right, very near start] {1} node[right, very near end] {0..*} node[left, midway] {run} (container.north);
        \end{tikzpicture}
    \end{minipage}
...
\end{mintedbox}
\caption{Script sh per addestramento di un modello su molteplici subset di SciLay.}
\label{lst:train-script-sh}
\end{customcode}
...
\begin{itemize}
    ...
    %LR: le epoche non contano con la memoria occupata, ma solo col tempo
    %\item La riduzione del numero di epoche. 
